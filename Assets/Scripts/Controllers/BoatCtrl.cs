...
public class BoatCtrl : ClickAction
{
    ...
    public Vector3 AddRole(Role roleModel) {
        int index = -1;
        if (boatModel.onBoat[0] == null) index = 0;
        else if (boatModel.onBoat[1] == null) index = 1;
        if (index == -1) return roleModel.role.transform.localPosition;

        boatModel.onBoat[index] = roleModel;
    ...
    public void RemoveRole(Role roleModel) {
        for (int i = 0; i < 2; ++i){
            if (boatModel.onBoat[i] == roleModel) {
                boatModel.onBoat[i] = null;
                if (roleModel.isPriest) boatModel.priestCount--;
                else boatModel.devilCount--;
                break;
            }
    ...
    public void DealClick() {
        if (boatModel.onBoat[0] != null || boatModel.onBoat[1] != null) {
            userAction.MoveBoat();
        }
    }
}